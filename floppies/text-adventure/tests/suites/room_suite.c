...
START_TEST(test_init_room)
{
    ...
    ck_assert_ptr_ne(room.items, NULL);
    ck_assert_int_eq(room.num_items, 0);
    ck_assert_int_eq(room.max_items, 4);
    free_room(&room);
}
END_TEST
...
START_TEST(test_free_room)
{
    ...
    ck_assert_ptr_eq(room.items, NULL);
    ck_assert_int_eq(room.num_items, 0);
    ck_assert_int_eq(room.max_items, 0);
}
END_TEST
...
START_TEST(test_add_item_to_room)
{
    ...
    ck_assert_int_eq(room.num_items, 1);
    ck_assert_int_eq(room.items[0], 42);
    for (int i=0; i<10; i++)
    {
        add_item_to_room(&room, i);
        ck_assert_int_eq(room.num_items, i+2);
    }
    ck_assert_int_eq(room.max_items,16);
    free_room(&room);
}
END_TEST
...
START_TEST(test_remove_item_from_room)
{
    ...
    ck_assert_int_eq(room.num_items, 10);
    ...
    for (int i=0; i<10; i++)
    {
        ...
        ck_assert_int_eq(room.num_items, 9-i);
    }
    ck_assert_int_eq(room.max_items, 16);
    remove_item_from_room(&room, 42);
    ck_assert_int_eq(room.num_items, 0);
